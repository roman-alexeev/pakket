...
sub install_package {
    ...
    my $pkg_verion    = $package->version;
    my $pkg_cat_name  = $package->cat_name;
    my $pkg_full_name = $package->full_name;

    $log->debugf( "About to install %s (into $dir)", $full_name );

    if ( my $version = $installed->{$pkg_cat}{$pkg_name} ) {
        if ( $version ne $pkg_version ) {
            $log->critical(
                "$pkg_cat_name=$version already installed. "
              . "Cannot install new version: $pkg_verion"
            );

            exit 1;
        }

        $log->debug("$full_name already installed.");
